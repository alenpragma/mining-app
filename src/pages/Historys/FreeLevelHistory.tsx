...
const FreeLevelHistory = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const { data } = await axiosInstance.get(
        `/admin/free-level-history?per_page=${perPage}&page=${currentPage + 1}`,
      );

      setDatas(data.free_level_bonus_history);
      setLoading(false);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Level History" />
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-gray-2 text-left dark:bg-meta-4">
                  ...
                  <TableHead data="Description" cN="min-w-[140px]" />
                </tr>
              </thead>
              <tbody>
...
export default FreeLevelHistory;
