  ...
  const fetchData = async () => {
    ...
    try {
      const { data } = await axiosInstance.get(
        `/admin/a2i-token-history?per_page=${perPage}&page=${currentPage + 1}`,
      );

      setDatas(data.daily_a2i_bonus_history);
      setLoading(false);
    } catch (error) {
      setLoading(false);
...
export default A2ITokenHistory;
